	...
	$sqlDetail = "SELECT * from album_photo WHERE photoID = ".$photoid;
			...
			while ($Detail = mysqli_fetch_assoc($rsDetail)) {
				...
				while ($ususer = mysqli_fetch_assoc($rsUser)) {
					echo "<script>alert({$ususer['username']});</script>";
				...
				while ($CoComment = mysqli_fetch_assoc($rsComment)) {
								...
								while ($ususer1 = mysqli_fetch_assoc($rsUser1)) {
									...
									if($myself == $CoComment['userID']){
										
									}else{
										echo"	<a class='various' data-fancybox-type='iframe' href='reportComment.php?commentID={$CoComment['commentID']}&baduser={$CoComment['userID']}' id='report'>
											<i class='fa fa-exclamation-circle'></i><span id='report'>Report</span>
										</a>";
									}
